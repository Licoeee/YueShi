...
interface MerchantAccountBookPageData extends RolePageSceneSwitchState {
  activeRole: RoleType
...
>({
  data: {
    activeRole: 'merchant',
    scenePath: '/pages/merchant/account-book/account-book',
    sceneDirection: 'none',
    sceneMotionTick: 0,
  ...
  onShow() {
    const nextState = buildPreviewableRolePageState(getApp<IAppOption>().globalData.roleSession, 'merchant')
